...
module.exports = async function (source) {
    ...
    if (curr.attributes) {
      ...
      for (const attr of curr.attributes) {
        if (attr.key.content.startsWith('ac-')) {
          needInject = true;
          magicContent.overwrite(attr.key.startPosition, attr.endWrapper.endPosition + 1, '');

          if (!sheetRegistries[usedIndex]) {
            sheetRegistries[usedIndex] = {};
          }
          sheetRegistries[usedIndex][attr.key.content.slice(3)] = attr.value.content;
      ...
      if (needInject) {
        ...
        let endPos = -1;
        const cls = `class="${classAttrNode?.value.content ?? ''} {{ __θac${usedIndex} }}"`;
        if (classAttrNode) {
  ...
  if (script) {
    ...
    magicContent.appendRight(basePos + 1, `\r\n${sheetRegistries.map((iter, index) => `const __θac${index}Registry=${JSON.stringify(iter)};`).join('\r\n')}`);
    ...
    magicContent.appendRight(endPos, `, { ${Array(usedIndex).fill().map((_, i) => `__θac${i}: ''`).join(',')} });`);
